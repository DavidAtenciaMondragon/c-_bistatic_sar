...
int main() {
    ...
    Target strTarget = initializeTarget();  // Initializes target at origin (0,0,0)
    System strSystem = initializeSystem();
    
    // Update target position to the desired coordinates
    updatePointToProc(strTarget, 20.0, 0.0, -5.0);
    
    std::cout << "Target initialized at origin and updated to position: (" 
              << strTarget.pos[0] << ", " << strTarget.pos[1] << ", " << strTarget.pos[2] << ")" << std::endl;
    ...
    exportProcessedData(ProjectPaths::OUTPUT_DIR, rawResult, trajectories, processedDEM, grid, strEnvironment);
    
    // Generate processing report
    generateProcessingReport(ProjectPaths::OUTPUT_DIR, strRadarTx, strRadarRx, strTarget, strSystem, 
    ...
    std::cout << "Check '" << ProjectPaths::PROC_DIR << "/' folder for processed data files." << std::endl;
